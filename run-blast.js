...
async function main() {
  ...
  await mongo.connect();
  const registrations = await mongo.db().collection('registrations')
  ...
  console.log(`\nIniciando blast — ${total} destinatarios\n`);
  ...
  const transporter = nodemailer.createTransport({
    ...
    auth: { user: USER_EMAIL, pass: PASS_EMAIL },
  });
  ...
  for (const r of registrations) {
    try {
      ...
      sent++;
      process.stdout.write(`\r[${sent + failed}/${total}] ✓ ${sent} enviados  ✗ ${failed} fallidos`);
    } catch (err) {
      failed++;
